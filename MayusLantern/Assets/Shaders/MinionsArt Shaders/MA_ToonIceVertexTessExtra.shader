Shader "MinionsArt/Toon/Lit Ice VertexColor Tessellated Extra" {
	Properties{
		[Header(Main)]	
		...
		_NormalSnow("Normal Snow", 2D) = "bump" {}
		_NormalIce("Normal Ice", 2D) = "bump" {}
		_NormalScale("Normal Scale Snow", Range(0,1)) = 1
		_NormalScaleIce("Normal Scale Ice", Range(0,1)) = 1
		[Header(Tesselation)]
		...
		_IceCutoffOffset("Ice Cutoff Offset", Range(0.1,1)) = 0.1
	}
		...
		sampler2D _MainTex, _Noise, _BumpMap, _NormalSnow, _NormalIce;
		...
		float _SnowCutoff, _IceCutoff, IceVertexCutoff, _IceCutoffOffset;
		float _NormalScale, _NormalScaleIce;
		...
		void surf(Input IN, inout SurfaceOutput o) {			
			...
			// normal noise triplanar for x, y, z sides
			float3 xnn = UnpackNormal(tex2D(_NormalSnow, IN.worldPos.zy * _NormalScale));
			float3 ynn = UnpackNormal(tex2D(_NormalSnow, IN.worldPos.zx * _NormalScale));
			float3 znn = UnpackNormal(tex2D(_NormalSnow, IN.worldPos.xy * _NormalScale));

			// lerped together all sides for noise texture
			float3 normalnoise = znn;
			normalnoise = lerp(normalnoise, xnn, blendNormal.x);
			normalnoise = lerp(normalnoise, ynn, blendNormal.y);

			// normal noise triplanar for x, y, z sides
			float3 xnni = UnpackNormal(tex2D(_NormalIce, IN.worldPos.zy * _NormalScaleIce));
			float3 ynni = UnpackNormal(tex2D(_NormalIce, IN.worldPos.zx * _NormalScaleIce));
			float3 znni = UnpackNormal(tex2D(_NormalIce, IN.worldPos.xy * _NormalScaleIce));

			// lerped together all sides for noise texture
			float3 normalnoiseIce = znni;
			normalnoiseIce = lerp(normalnoiseIce, xnni, blendNormal.x);
			normalnoiseIce = lerp(normalnoiseIce, ynni, blendNormal.y);
			
			// main texture
			half4 c = tex2D(_MainTex, IN.uv_MainTex) * _Color; 
			// part of model facing bottom
			float facingBottom =1- smoothstep(_IceCutoff * noisetexture.r, (_IceCutoff + _IceCutoffOffset)* noisetexture.r, worldNormal.y);
			...
			// normals
			float3 normalBase = UnpackNormal(tex2D(_BumpMap, IN.uv_MainTex)) * (1-snowEdge);
			float3 normalSnow = normalnoise * (1-facingBottom)  * snowEdge;
			float3 normalIce = normalnoiseIce *facingBottom * snowEdge;

			o.Normal = normalBase + normalSnow + normalIce;

			// rim lighting
			half rim = 1.0 - saturate(dot(normalize(IN.viewDir), o.Normal)); // rimlight

			// ice rim
			float3 iceRim = _RimColor.rgb *pow(rim, _RimPowerIce)* facingBottom* (step(0.25, IN.color.r));
			// snow rim
