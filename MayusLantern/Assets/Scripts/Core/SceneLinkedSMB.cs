namespace ML.Core
{
    ...
    public class SceneLinkedSMB<TMonoBehaviour> : SealedSMB where TMonoBehaviour : MonoBehaviour
    {
        ...
        {
            //If the gameobject isn't active, don't update the state
            if (!animator.gameObject.activeSelf) { return; }

            //If the animator is in transition, and the next state equals the intented state
            ...
            //If the animator is NOT transitioning, and the first frame of the state has happened
            ...
            //If the animator is in transition, and the first frame has already happened
            ...
            //If the animator is NOT in transition, and the first frame hasn't happened
            if (!animator.IsInTransition(layerIndex) && !m_FirstFrameHappened)
            {
            ...
            //If the animator is in transition and the current state is the intented state
        ...
        /// <summary>
        /// Called by a Monobehaviour in the scene during the Start function
        /// </summary>
        public virtual void OnStart(Animator animator) { }

        /// <summary>
        /// Called before Update when execution of the state first starts (on transition to the state).
        /// </summary>
        ...
        /// Called on the first frame after the transition to the state has finished.
        /// </summary>
        public virtual void OnSLStatePostExit(Animator animator, AnimatorStateInfo stateInfo, int layerIndex) { }

        /// <summary>
        /// Called on the first frame after the transition to the state has finished.
        /// </summary>
        public virtual void OnSLStatePostExit(Animator animator, AnimatorStateInfo stateInfo, int layerIndex, AnimatorControllerPlayable controller) { }
    ...
    /// <summary>
    /// This class replaces the normal StateMachineBehaviour class.
    /// It adds the possibility of having direct references to the object the
    /// state is running on, avoiding retreiving it through GetComponent every
    /// time. Cost saving class!
    /// </summary>
